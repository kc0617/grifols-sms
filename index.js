...
async function handleInbound(req, res) {
  ...
  if (!from) {
    console.warn("[Inbound] No sender number — skipping.");
    return;
  }
  ...
  if (STOP_KEYWORDS.includes(text)) {
    ...
    await sendSMS(from, "You have been unsubscribed and will no longer receive SMS messages from Grifols Chattanooga Plasma Center. If this was a mistake, please call us at (423) 855-1085.");
    return;
  }
  ...
  try {
    switch (session.step) {
      case "idle":   await handleIdle(from, text, session); break;
      case "suggest": await handleSuggest(from, text, session); break;
      case "alt_date": await handleAltDate(from, text, session); break;
      case "alt_time": await handleAltTime(from, text, session); break;
      case "done":   await handleDone(from, session); break;
      default:       await handleIdle(from, text, session);
    }
  } catch (err) {
...
async function handleIdle(phone, text, session) {
  ...
  if (donor.status === "deferred") {
    const until = new Date(donor.deferralUntil + "T12:00:00").toLocaleDateString("en-US", { month: "long", day: "numeric" });
    await sendSMS(phone, `Hi ${donor.firstName}! This is Jessi at the Grifols Chattanooga Plasma Center.\n\nYour account shows a temporary deferral (${donor.deferralReason}) until ${until}. You'll be able to schedule again after that date. Please call us at ${center.phone} with any questions.\n\n${OPT_OUT_NOTICE}`);
    return;
  }
  ...
  await sendSMS(phone, `Hi ${donor.firstName}! This is Jessi at the Grifols Chattanooga Plasma Center - thank you for being a valued donor!\n\nBased on your donation history, we suggest your next appointment on:\nDate: ${formatDate(suggested.date)}\nTime: ${suggested.time}\n\nReply YES to confirm, or NO to choose a different date.\n\n${OPT_OUT_NOTICE}`);
}
...
async function handleSuggest(phone, text, session) {
  ...
  if (["yes", "y", "confirm", "ok", "sure"].includes(text)) {
    ...
    await sendSMS(phone, `Perfect, ${donor.firstName}! Your appointment is confirmed:\n\nDate: ${formatDate(suggested.date)}\nTime: ${suggested.time}\nLocation: ${center.name}\nPhone: ${center.phone}\n\nWe'll see you then! Thank you for saving lives.\n- Jessi\n\n${OPT_OUT_NOTICE}`);
    ...
    const dateList = alternateDates.map((d, i) => `${i + 1}. ${formatDate(d)}`).join("\n");
    await sendSMS(phone, `No problem! Here are some other available dates at ${center.name}:\n\n${dateList}\n\nReply 1, 2, 3, or 4 to choose a date.`);

  } else {
    await sendSMS(phone, `Please reply YES to confirm your appointment on ${formatDate(suggested.date)} at ${suggested.time}, or NO to choose a different date.`);
  }
}
...
async function handleAltDate(phone, text, session) {
  ...
  if (isNaN(idx) || idx < 0 || idx >= alternateDates.length) {
    await sendSMS(phone, `Please reply with a number between 1 and ${alternateDates.length} to select a date.`);
    return;
  }
...
async function handleAltTime(phone, text, session) {
  ...
  if (isNaN(idx) || idx < 0 || idx >= altSlots.length) {
    await sendSMS(phone, `Please reply with a number between 1 and ${altSlots.length} to select a time.`);
    return;
  }

  const chosenTime = altSlots[idx];
  updateSession(phone, { step: "done", booking: { date: selectedDate, time: chosenTime } });
  await sendSMS(phone, `Confirmed, ${donor.firstName}! Here are your appointment details:\n\nDate: ${formatDate(selectedDate)}\nTime: ${chosenTime}\nLocation: ${center.name}\nPhone: ${center.phone}\n\nThank you for your donation - you're making a difference!\n- Jessi\n\n${OPT_OUT_NOTICE}`);
}
...
async function handleDone(phone, session) {
  ...
  await sendSMS(phone, `Hi ${donor.firstName}! Your appointment is already confirmed for ${formatDate(booking.date)} at ${booking.time}.\n\nTo make any changes, please call us at ${center.phone}. See you soon!\n- Jessi`);
}
